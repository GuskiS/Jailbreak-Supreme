...
public plugin_init()
{
	...
	DisableHamForward((g_pHamHooks[0] = RegisterHam(Ham_TraceAttack, "player", "Ham_TraceAttack_pre", 0)));
	DisableHamForward((g_pHamHooks[1] = RegisterHam(Ham_TakeDamage, "player", "Ham_TakeDamage_pre", 0)));
	DisableHamForward((g_pHamHooks[2] = RegisterHam(Ham_TakeDamage, "player", "Ham_TakeDamage_post", 1)));
	DisableHamForward((g_pHamHooks[3] = RegisterHam(Ham_Item_Deploy, "weapon_knife", "Ham_Item_Deploy_post", 1)));
	DisableHamForward((g_pHamHooks[4] = RegisterHam(Ham_Killed, "player", "Ham_Killed_pre", 0)));

#if defined JAIL_HAMBOTS
	DisableHamForwardBots((g_pHamBotHooks[0] = RegisterHamBots(Ham_TraceAttack, "Ham_TraceAttack_pre", 0)));
	DisableHamForwardBots((g_pHamBotHooks[1] = RegisterHamBots(Ham_TakeDamage, "Ham_TakeDamage_pre", 0)));
	DisableHamForwardBots((g_pHamBotHooks[2] = RegisterHamBots(Ham_Killed, "Ham_Killed_pre", 0)));
#endif
...
my_registered_stuff(val)
{
	if(val)
	{
		...
		#if defined JAIL_HAMBOTS
		for(new i = 0; i < sizeof(g_pHamBotHooks); i++)
			EnableHamForwardBots(g_pHamBotHooks[i]);
		#endif
	}
	else
	{
		...
		#if defined JAIL_HAMBOTS
		for(new i = 0; i < sizeof(g_pHamBotHooks); i++)
			DisableHamForwardBots(g_pHamBotHooks[i]);
		#endif
	}
}