...
new g_HamBotsTraceAttack, g_HamBotsKilled;
#endif
...
public plugin_init()
{
	...
	DisableHamForward((g_HamTraceAttack = RegisterHam(Ham_TraceAttack, "player", "Ham_TraceAttack_pre", 0)));
	DisableHamForward((g_HamKilled = RegisterHam(Ham_Killed, "player", "Ham_Killed_post", 1)));
#if defined JAIL_HAMBOTS
	DisableHamForwardBots((g_HamBotsKilled = RegisterHamBots(Ham_Killed, "Ham_Killed_post", 1)));
	DisableHamForwardBots((g_HamBotsTraceAttack = RegisterHamBots(Ham_TraceAttack, "Ham_TraceAttack_pre", 0)));
#endif
...
my_ham_hooks(val)
{
	if(val)
	{
		...
		#if defined JAIL_HAMBOTS
		EnableHamForwardBots(g_HamBotsTraceAttack);
		EnableHamForwardBots(g_HamBotsKilled);
		#endif
	}
	else
	{
		...
		#if defined JAIL_HAMBOTS
		DisableHamForwardBots(g_HamBotsTraceAttack);
		DisableHamForwardBots(g_HamBotsKilled);
		#endif
	}
}
